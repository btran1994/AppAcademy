...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    ...
    new_info = "get_#{name}_info"
      #define_method(new_info) {@data[@box_id][name]["info"]}

    new_tastiness = "get_#{name}_tastiness"
      #define_method(new_tastiness) {@data[@box_id][name]["tastiness"]}


    info = @snack_box.send(new_info.to_sym, @box_id)

    tastiness = @snack_box.send(new_tastiness.to_sym, @box_id)
